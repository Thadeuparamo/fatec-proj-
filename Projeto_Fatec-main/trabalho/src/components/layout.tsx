...
import { Briefcase, Menu, User, X, HelpCircle, Leaf } from "lucide-react";
import { useEffect, useState } from "react";
import { getSessionUser, normalizeTipo } from "../services/auth";
...
export function Layout() {
  ...
  const userTipo = normalizeTipo(user?.tipo);

  useEffect(() => {
    setMobileMenuOpen(false);
  }, [location.pathname]);

  const clientAreaLink = user ? "/dashboard" : "/acesso?tipo=cliente&redirect=/dashboard";
  const providerAreaLink = user ? "/prestador" : "/acesso?tipo=prestador&redirect=/prestador";

  // Mostrar botão somente se não logado, ou se for do tipo correspondente
  const showClientButton = !user || userTipo === "cliente" || userTipo === "admin";
  const showProviderButton = !user || userTipo === "prestador" || userTipo === "admin";
          ...
          {mobileMenuOpen && (
            <div className="animate-enter md:hidden py-4 space-y-4">
              ...
              <div className="flex flex-col gap-2 pt-4">
                {showClientButton && (
                  <Link to={clientAreaLink} onClick={() => setMobileMenuOpen(false)}>
                ...
                {showProviderButton && (
                  <Link to={providerAreaLink} onClick={() => setMobileMenuOpen(false)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-300">
                ...
                <li>
                  <a href="#" className="hover:text-orange-300">Vantagens</a>
                </li>
                <li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-300">
                ...
                <li>
                  <a href="#" className="hover:text-orange-300">Privacidade</a>
                </li>
              </ul>
            </div>
          </div>
